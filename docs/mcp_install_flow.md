...
- 如果企业内网 OA 只能使用 HTTP，MCP 会先返回 `transportSecurityRequired=true`、确认文案、一次性确认令牌和 `nextAction`。Agent 必须先让用户确认“确认使用不安全连接授权”，确认后再按 `nextAction` 调用 `oa_begin_auth(insecure=true)`，不需要用户手动改 MCP 配置或重启。确认令牌只给 Agent 内部传递，不展示给用户。
- HTTPS 跳过证书校验不走普通用户确认流程，需要管理员显式设置 `OA_AGENT_ALLOW_INSECURE_AUTH=1`。
- `OA_AGENT_ALLOW_INSECURE_AUTH=1` 只作为管理员预先批准可信内网 HTTP 或 HTTPS 跳过证书校验的全局例外；普通安装流程不需要设置。

MCP 只保存登录后的 cookie、baseUrl、登录账号身份提示和待确认审批状态：
...
- 如果返回里有 `transportSecurityRequired=true` 和 `nextAction`，Agent 必须先提示用户确认当前 OA 是可信公司内网地址。用户回复“确认使用不安全连接授权”后，再按 `nextAction` 继续发起本机授权；不要把确认令牌展示给用户。
