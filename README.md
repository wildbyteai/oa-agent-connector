...
这个连接器让 Agent 可以帮用户使用 OA：查看待办清单、查看申请详情、在用户明确确认后提交单条或批量审批；也可以搜索 OA 文档、查看文档详情、下载当前账号有权限看到的附件。
...
Agent 会先逐条校验并列出完整确认清单。单次最多 20 条，可以混合同意和驳回；用户确认后才会按清单顺序逐条提交。
...
- 审批前必须展示单据、动作、备注，让用户确认；批量审批必须逐条展示。
