...
当前稳定版本是 `0.2.13`，审批能力范围固定为 `standard-approval-v1`。本版本不填写或修改业务表单，也不选择下一流程节点。
...
11. 每次 OA 业务操作完成后，按返回的 `versionCheck` 调用 `oa_version_status`。没有更新或检查失败时不要打扰用户；只有发现新版本时才询问用户是否升级，用户确认后才能执行升级。
...
每个 OA 业务工具的结果都会带一个给 Agent 使用的 `versionCheck` 提示。Agent 应在本次业务结果正常展示后调用 `oa_version_status`，普通调用不要设置 `force=true`。

- 已是最新版：不对用户输出任何版本信息。
- 暂时无法检查：不影响当前 OA 操作，也不对用户显示技术错误。
- 发现新版本：告诉用户当前版本和新版本，并询问是否升级。
- 用户没有明确同意：不能自动安装、升级或重启 MCP。
- 用户确认升级后：按本仓库安装命令升级，并刷新或重启 Agent 客户端的 MCP 工具。

版本检查只读取公开 GitHub 仓库中的版本文件，不会发送 OA 地址、账号、Cookie 或业务数据。成功结果会在本机缓存 24 小时，失败结果缓存 1 小时。公司环境如需完全关闭，可设置 `OA_AGENT_DISABLE_UPDATE_CHECK=1`。
...
- `OA_AGENT_DISABLE_UPDATE_CHECK=1` 可关闭公开 GitHub 版本检查；关闭后不影响 OA 查询、详情、下载和审批。
...
- 不修改 OA 服务端。
- 不绕过 OA 权限。
- 每次业务操作后按 `versionCheck` 检查版本；只有 `updateAvailable=true` 时才提示用户，升级前必须取得用户明确确认。
